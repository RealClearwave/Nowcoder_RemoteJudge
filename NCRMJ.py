...
if '-h' in argv or '--help' in argv or len(argv) == 1:
    ...
    print(
        "  -s [username]\t\tShow the status of the user in the contest")
    print("Example:")
...
def submit_code(problemName, code, languageID, languageName):
    ...
    data = {
        ...
        'doneQuestionId': problems["contestID"],
    }
    ...
    while True:
        url = f"https://ac.nowcoder.com/nccommon/status?submissionId={
            x['data']}&tagId={data['tagId']}&subTagId={data['subTagId']}"
...
if '-s' in argv:
    contest_id = problems["contestID"]
    user_name = argv[argv.index('-s')+1]
    r = requests.get(f"https://ac.nowcoder.com/acm-heavy/acm/contest/real-time-rank-data?token=&id={
                     contest_id}&searchUserName={user_name}&limit=0", cookies=cookies)
    x = json.loads(r.text)
    x = x['data']['rankData'][0]
    print(f"\033[34mRank: {x['ranking']} Accepted: {x['acceptedCount']} Score:{
          x['totalScore']} Penalty: {x['penaltyTime']//1000}")
    print("Details:")

    scoreList = x['scoreList']

    for i in range(len(scoreList)):
        print(f"{"\033[92m" if scoreList[i]['accepted']
              == True else "\033[91m"}", end="")
        print(f"Problem: {chr(65+i)} {"Accepted" if scoreList[i]['accepted'] == True else f'Unaccepted(-{
              scoreList[i]['failedCount']})'} Score: {scoreList[i]['score']}")
...
print("\033[34mScore: {}%: [".format(resHundredRate), "▋" *
      (resHundredRate // 2), ' ' * ((100-resHundredRate)//2), ']')
print(f"{"\033[92m" if resHundredRate ==
      100 else "\033[91m"}Description: {resDesc}")
print("\033[0m")
